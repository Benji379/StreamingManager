...
public class ExportarExcel {
    ...
    public static void exportToExcel(JTable table, String filePath, Color headerBackgroundColor) {
        ...
        // Iterar a travÃ©s de las filas de la tabla
        for (int rowIndex = 0; rowIndex < table.getRowCount(); rowIndex++) {
            ...
            for (int columnIndex = 0; columnIndex < table.getColumnCount(); columnIndex++) {
                ...
                if (cellValue != null) {
                    ...
                    excelCell.setCellValue(""); // Si el valor es nulo, se asigna una cadena vacÃ­a
                }
        ...
        // Ajustar automÃ¡ticamente las filas
        for (int rowIndex = 0; rowIndex <= table.getRowCount(); rowIndex++) {
            XSSFRow row = sheet.getRow(rowIndex);
            if (row != null) {
                row.setHeight((short) -1); // Establecer altura automÃ¡tica para todas las filas
            }
        }

        // Aplicar el filtro automÃ¡tico
    ...
    public static void exportToExcel(DefaultTableModel modeloTabla, String filePath, Color headerBackgroundColor) {
        XSSFWorkbook workbook = new XSSFWorkbook();
        XSSFSheet sheet = workbook.createSheet("Data");
        ...
        for (int columnIndex = 0; columnIndex < modeloTabla.getColumnCount(); columnIndex++) {
            XSSFCell headerCell = headerRow.createCell(columnIndex);
            headerCell.setCellValue(modeloTabla.getColumnName(columnIndex));
            headerCell.setCellStyle(headerStyle);
        }

        // Iterar a través de las filas del modelo de tabla
        for (int rowIndex = 0; rowIndex < modeloTabla.getRowCount(); rowIndex++) {
            XSSFRow excelRow = sheet.createRow(rowIndex + 1);
            ...
            for (int columnIndex = 0; columnIndex < modeloTabla.getColumnCount(); columnIndex++) {
                XSSFCell excelCell = excelRow.createCell(columnIndex);
                Object cellValue = modeloTabla.getValueAt(rowIndex, columnIndex);
        ...
        XSSFAutoFilter filter = sheet.setAutoFilter(new CellRangeAddress(0, modeloTabla.getRowCount(), 0, modeloTabla.getColumnCount() - 1));
